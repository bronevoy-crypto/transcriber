...
| Модель | WER* | Примечание |
|--------|------|------------|
| `gigaam_e2e` | 6.8% | Рекомендуется для русского. Пунктуация, числа цифрами, word-level тайминги. |
| `gigaam` | 8–10% | Чуть быстрее e2e, без пунктуации, без пословных таймингов. |
| `whisper` | 10–15% | large-v3, мультиязычный, нужна GPU 8+ ГБ. Word-level есть. |
| `parakeet` | 25.7% | 25 языков включая RU. Тайминги приблизительные (из BPE токенов). |
| `tone` | 27.4% | Заточен под телефонию 8 kHz, без пунктуации. |
